...
public class UserApiControllerTest {
	...
	public void should_create_new_user() throws Exception {
		...
		this.mockMvc.perform(
				post("/api/users").contentType(MediaType.APPLICATION_JSON)
						.content(JsonUtil.toJson(user))).andExpect(
				status().isCreated());
		assertThat(userRepository.findAll()).extracting(User::getEmail)
	...
	public void should_delete_user() throws Exception {
		this.mockMvc.perform(
				delete("/api/user/bob@spring.io").accept(
	...
	public void should_return_not_found_when_deleting_unknown_user()
			throws Exception {
		this.mockMvc.perform(
				delete("/api/user/non-existing@mail.com").accept(
	...
	public void put_should_update_existing_user() throws Exception {
		...
		this.mockMvc.perform(
				put("/api/user/bob@spring.io").content(JsonUtil.toJson(user))
